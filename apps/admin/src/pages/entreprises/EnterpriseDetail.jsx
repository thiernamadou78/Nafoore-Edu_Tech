    ...
    <div className="max-w-5xl">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[300px_1fr]">
        ...
        <div className="space-y-6 lg:sticky lg:top-6 lg:self-start">
          <Card className="p-6">
            <h1 className="text-lg font-semibold text-gray-900">{enterprise.raisonSociale}</h1>
            <div className="mt-3 space-y-1 text-sm text-gray-600">
              {enterprise.siret && <p>SIRET {enterprise.siret}</p>}
              {enterprise.secteurActivite && <p>{enterprise.secteurActivite}</p>}
              {enterprise.emailDomain && <p>@{enterprise.emailDomain}</p>}
            </div>
            <div className="mt-4 flex flex-wrap gap-1.5 border-t border-gray-100 pt-4">
              <Badge tone="gray">{enterprise.contracts.length} contrat(s)</Badge>
              <Badge tone="gray">{enterprise.rhAccounts.length} compte(s) RH</Badge>
            </div>
          </Card>
        </div>
                  ...
                  {enterprise.contracts.map((contract) => (
                    <li key={contract.id} className="py-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            Du {formatDate(contract.dateDebut)} au{' '}
                            {formatDate(contract.dateExpiration)}
                          </p>
                        </div>
                      ...
                      {contract.amendments.length > 0 && (
                        <div className="mt-2">
                          <Timeline
                            items={contract.amendments}
                            renderItem={(amendment) => (
                              <p className="text-xs text-gray-500">
                                {AMENDMENT_TYPE_LABELS[amendment.type] ?? amendment.type} : «{' '}
                                {amendment.ancienneValeur ?? '—'} » → « {amendment.nouvelleValeur}{' '}
                                » le {formatDate(amendment.dateEffet)} par{' '}
