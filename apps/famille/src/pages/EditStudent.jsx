...
export function EditStudent() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (photoFile) {
        ...
        await api.upload(`/family/students/${id}/photo`, formData).catch(() => {})
      }
      await refresh()
      navigate(`/eleves/${id}`, { replace: true })
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <h1 className="mb-6 font-serif text-2xl font-bold text-navy">Modifier les infos</h1>
      <Card className="p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select value="" onChange={(e) => addSubject(e.target.value)} className={inputClass}>
          ...
          <div className="flex gap-2">
            <Link to={`/eleves/${id}`} className="flex-1">
              <Button type="button" variant="secondary" className="w-full">
                Annuler
              </Button>
            </Link>
            <Button type="submit" icon={Save} loading={submitting} className="flex-1">
              Enregistrer
            </Button>
          </div>
        </form>
