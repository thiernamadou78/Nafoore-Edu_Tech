...
// Gabarit generique "Bonjour X, ..." : evite un fichier de template par
// message court (renouvellements, etc.).
...
}: NoticeEmailInput): string {
  const body = paragraphs
    .map(
      (paragraph) =>
        `<p style="margin:0 0 16px 0;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#374151;">${escapeHtml(paragraph)}</p>`,
    )
    .join('');
  const cta = ctaUrl
    ? `<tr>
                ...
                <table role="presentation" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="border-radius:999px;background-color:#EAB308;">
                      <a href="${escapeHtml(ctaUrl)}" style="display:inline-block;padding:12px 28px;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:bold;color:#1E3A8A;text-decoration:none;border-radius:999px;">
                        ${escapeHtml(ctaLabel)}
                      </a>
                    </td>
                  </tr>
...
<html lang="fr">
  <head>
    ...
    <title>${escapeHtml(label)} — Nafoore Education</title>
  </head>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(30,58,138,0.08);">
            ...
            <tr>
              <td style="padding:36px 32px 8px 32px;">
                <p style="margin:0 0 4px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:bold;letter-spacing:0.08em;text-transform:uppercase;color:#EAB308;">${escapeHtml(label)}</p>
            ...
            <tr>
              <td style="padding:28px 32px 28px 32px;">
                <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.6;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:16px;">
                  Pour toute question, contactez l'équipe Nafoore Education.
                </p>
              </td>
            </tr>
          </table>
