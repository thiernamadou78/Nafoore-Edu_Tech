export interface SessionCancelledEmailInput {
  studentName: string;
  teacherName: string;
  sessionDate: string;
  reason: string;
...
export function renderSessionCancelledEmail({
  studentName,
  teacherName,
  sessionDate,
  reason,
  portalUrl,
}: SessionCancelledEmailInput): string {
  return `<!doctype html>
<html lang="fr">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Séance annulée — Nafoore Education</title>
  </head>
            ...
            <tr>
              <td style="padding:36px 32px 8px 32px;">
                ...
                <h1 style="margin:0 0 16px 0;font-family:Georgia,'Playfair Display',serif;font-size:24px;line-height:1.3;color:#1E3A8A;">
                  Séance annulée pour ${escapeHtml(studentName)}
                </h1>
                <p style="margin:0 0 20px 0;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#374151;">
                  ${escapeHtml(teacherName)} a annulé la séance prévue le <strong>${escapeHtml(sessionDate)}</strong>.
                </p>
              </td>
            </tr>
            <tr>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f3ef;border-radius:12px;">
                  <tr>
                    <td style="padding:20px 24px;">
                      <p style="margin:0 0 4px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:bold;letter-spacing:0.04em;text-transform:uppercase;color:#6b7280;">Motif</p>
                      <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#1E3A8A;">${escapeHtml(reason)}</p>
                    </td>
                  </tr>
                </table>
              </td>
            ...
            <tr>
              <td style="padding:20px 32px 0 32px;">
                <p style="margin:0 0 24px 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.6;color:#6b7280;">
                  Vous pouvez consulter le planning à jour depuis votre espace famille.
                </p>
            ...
            <tr>
              <td style="padding:32px 32px 28px 32px;">
                <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.6;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:16px;">
                  Pour toute question, contactez l'enseignant directement via la messagerie de votre espace famille.
                </p>
              </td>
            </tr>
          </table>
