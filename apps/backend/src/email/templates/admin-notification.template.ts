...
export function renderAdminNotificationEmail({
  ...
  lines,
  ctaUrl,
}: AdminNotificationEmailInput): string {
  const items = lines
    .map(
      (line) =>
        `<li style="margin:0 0 6px 0;">${escapeHtml(line)}</li>`,
    )
    .join('');
  const cta = ctaUrl
    ? `<tr>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(30,58,138,0.08);">
            <tr>
              <td style="background-color:#1E3A8A;padding:28px 32px;">
                <span style="font-family:Georgia,'Playfair Display',serif;font-size:20px;font-weight:bold;color:#ffffff;letter-spacing:0.02em;">Nafoore Education — Admin</span>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:36px 32px 8px 32px;">
                ...
                <ul style="margin:0 0 20px 0;padding-left:20px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.6;color:#374151;">
                  ${items}
                </ul>
              </td>
            </tr>
            ...
            <tr>
              <td style="padding:24px 32px 28px 32px;">
                <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.6;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:16px;">
                  Notification automatique de la plateforme Nafoore Education.
                </p>
              </td>
            </tr>
          </table>
